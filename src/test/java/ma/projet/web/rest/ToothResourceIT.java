...
import ma.projet.repository.ToothRepository;
...
class ToothResourceIT {
    ...
    void createTooth() throws Exception {
        ...
        restToothMockMvc
            .perform(
                post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(tooth))
            )
    ...
    void putWithMissingIdPathParamTooth() throws Exception {
        ...
        restToothMockMvc
            .perform(
                put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(tooth))
            )
    ...
    void deleteTooth() throws Exception {
        ...
        // Delete the tooth
        restToothMockMvc
            .perform(delete(ENTITY_API_URL_ID, tooth.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
